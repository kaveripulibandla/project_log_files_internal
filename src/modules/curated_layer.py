...
def log_curated_layer():
    ...
    df_curated = spark.read.option("header", True) \
        .csv(
        "C:\\project_log_files_internal\\src\\internal_files\\cleanse_log_file\\part-00000-01fb9e0d-3f28-4980-9e87-b0bff1c93910-c000.csv")
    ...
    remove_spec = df_curated.select('request', regexp_replace('request', '%|,|-|\?=', ''))
    ...
    log_agg_across_device.write.mode("overwrite").format('csv').option("header", True).save(
        "C:\\project_log_files_internal\\src\\internal_files\\log_agg_across_device_file")
